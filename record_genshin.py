...
def record_average_ru():
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# WRITE TO EXTERNAL FILE THE AVERAGE RU PER AMOUNT OF WISHES GIVEN THAT YOU CAP OUT AT 7
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# trials = 10000
# current_wishes = 656
# current_ru = 6
...
#     current_wishes = int(re.sub("[^0-9]", "", splitting[1]))+1
#     current_ru = float(re.sub("[^0-9.]", "", splitting[2]))
...
#       total_pulls = j
#       stats = WishStats(total_pulls, desired_five_stars, desired_ru, four_rateups, four_stars, rateups, standard_five_stars)
#       stats.run_stats(trials)
#       average_ru = stats.average_ru()
#       output_string+= str(average_ru) + "\n"
...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for column in column_description_list:
        ...
        if (length < 10):
            length = 10
        justification_list.append(length)
        string_column_description+=(column+",").rjust(length+1)
    ...
    if(timer and not exists(timer_filename)):
        with open(timer_filename, 'w') as f:
            f.write("Pity,".ljust(10) + "Time(s)   SKIP".ljust(10)+"\n")
    ...
    for j in range(current_wishes,1000):
        ...
        output_string = str(j).rjust(justification_list[0])
        ...
        with NoStdStreams():
            ...
            for k in range(0,8):
                #make sure things are spaced evenly
                output_string +=  "," + str(ratio_list[k]).rjust(justification_list[k+1])
        output_string += "\n"
        with open(filename, 'a') as f:
            f.write(output_string)
        if(timer):
            ...
            with open(timer_filename, 'a') as f:
                timer_string = (str(j)+",").ljust(10)+(str(time_diff)+"\n").ljust(10)
